	...
	const recentTransactions = [
		{
			...
			product: '2ct Round Diamond Ring',
			amount: 12500,
			date: '2023-11-28',
			status: 'completed',
		},
		{
			...
			product: '1.5ct Princess Diamond Earrings',
			amount: 8750,
			date: '2023-11-27',
			status: 'completed',
		},
		{
			...
			product: '3ct Diamond Tennis Bracelet',
			amount: 15200,
			date: '2023-11-26',
			status: 'processing',
		},
		...
		},
		{
			...
			product: '2.5ct Cushion Diamond Ring',
			amount: 18900,
			date: '2023-11-24',
			status: 'completed',
		},
				...
				<div className="animate-pulse">
					...
					<div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
						{[...Array(4)].map((_, i) => (
							<div
								key={i}
								className="bg-white shadow-lg rounded-lg p-6 h-32"
							></div>
						))}
					</div>
			...
			<div className="p-6">
							...
							<div>
								...
								<p className="text-xs text-green-500 mt-1 flex items-center">
									...
									>
										<path
											fillRule="evenodd"
											d="M12 7a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0V8.414l-4.293 4.293a1 1 0 01-1.414 0L8 10.414l-4.293 4.293a1 1 0 01-1.414-1.414l5-5a1 1 0 011.414 0L11 10.586l3.293-3.293A1 1 0 0114 7h-2z"
											clipRule="evenodd"
										/>
									</svg>
						...
						<div className="flex justify-between items-center">
							<div>
								...
								<p className="text-xs text-purple-500 mt-1 flex items-center">
									...
									{products?.length > 0
										? `${Math.floor(products.length * 0.15)} new`
										: '0 new'}
								</p>
							</div>
					...
					<div className="bg-white shadow-lg rounded-lg p-6 hover:shadow-xl transition-shadow">
						<div className="flex justify-between items-center mb-4">
							...
							<div className="flex space-x-2">
								<button className="text-xs bg-blue-100 text-blue-600 px-3 py-1 rounded-full">
									This Year
								</button>
								<button className="text-xs text-gray-500 hover:bg-gray-100 px-3 py-1 rounded-full">
									Last Year
								</button>
							</div>
						</div>
								...
								<div className="flex items-center">
									...
									<div>
										...
										<p className="text-sm text-gray-500">
											{diamonds?.length || 0} items
										</p>
									</div>
								</div>
								<span className="text-blue-600 font-semibold">
									${(diamonds?.length || 0) * 2500}
								</span>
							</div>
							<div className="flex justify-between items-center">
								<div className="flex items-center">
									...
									<div>
										...
										<p className="text-sm text-gray-500">
											{Math.floor((products?.length || 0) * 0.4)} items
										</p>
									</div>
								</div>
								<span className="text-purple-600 font-semibold">
									${Math.floor((products?.length || 0) * 0.4) * 1800}
								</span>
							</div>
							<div className="flex justify-between items-center">
								<div className="flex items-center">
									...
									<div>
										...
										<p className="text-sm text-gray-500">
											{Math.floor((products?.length || 0) * 0.3)} items
										</p>
									</div>
								</div>
								<span className="text-green-600 font-semibold">
									${Math.floor((products?.length || 0) * 0.3) * 1500}
								</span>
							</div>
							<div className="flex justify-between items-center">
								<div className="flex items-center">
									...
									<div>
										...
										<p className="text-sm text-gray-500">
											{Math.floor((products?.length || 0) * 0.2)} items
										</p>
									</div>
								</div>
								<span className="text-amber-600 font-semibold">
									${Math.floor((products?.length || 0) * 0.2) * 1200}
								</span>
							</div>
							<div className="flex justify-between items-center">
								<div className="flex items-center">
									...
									<div>
										...
										<p className="text-sm text-gray-500">
											{Math.floor((products?.length || 0) * 0.1)} items
										</p>
									</div>
								</div>
								<span className="text-red-600 font-semibold">
									${Math.floor((products?.length || 0) * 0.1) * 2000}
								</span>
							</div>
						</div>
				...
				<div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
					<div className="bg-white shadow-lg rounded-lg p-6 hover:shadow-xl transition-shadow">
						...
						<div className="space-y-4">
							<div className="flex items-center">
								<div className="w-12 h-12 bg-gray-200 rounded-lg mr-4 flex items-center justify-center text-xl">
									💍
								</div>
